...
mle <- function(pars, f_g, m_g) { 
  alpha <- pars[1]
  betaa <- pars[2]
  if (length(which(m_g==0))>0){
	fres <- -sum( (f_g/2 -1)*log(m_g+0.001) + (f_g/2)*log(f_g/2) + alpha*log(betaa) + lgamma(f_g/2+alpha) - lgamma(f_g/2) - lgamma(alpha) - (f_g/2 +alpha)*log(m_g*f_g/2+betaa))
  } else {
...
getInitialVals <- function(G.in, n.in, Xc.in, Y.in, SNP.in,  p.0,   q.in){
  ...
  compS <- function(y, n, H) {
      A <- crossprod(y, diag(1,n) - H) #t(y) %*% (diag(1,n) - H)
	  Asqd <- crossprod(A)
	  return(Asqd) 
    }
...
runHEFTmix <- function(G.in, n.in, Xc.in, Y.in, SNP.in, mu.0, sig20.0, sig21.0, betac.0, p.0, er_tol.in, q.in){
  ...
  while (is.na(er)==FALSE & er > er_tol.in & k < 10^5) {
     ...
     F_g.1 <- matrix(sapply(matout, function(m) m[6:{6+q.in-1}], simplify=TRUE), nrow=q.in, ncol=G.in) # when q.in=1, F_g, a vector, has trouble in sweep()
   ...
   HEFTmix = list( er.all=er.all, lc=lc,  
                mu = mu, sig20 = sig20, sig21 = sig21, Lam = Lam, sig2_g = sig2_g, #betac = betac,
				b_g = b_g, p = p, psi = psi,
